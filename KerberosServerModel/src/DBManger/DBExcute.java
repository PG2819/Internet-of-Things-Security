...
public class DBExcute {
	...
	public static int register(String userName, String passwd) {
		sql = "insert into user(UserName,Passwd) values ('" + userName + "','"
				+ passwd + "')";
		DBManger.DataConn.OpenConn();
		int re = DBManger.DataConn.executeUpdate(sql);
		if (re == 0) {
		...
		String sql2 = "select UserID from user where UserName='" + userName
				+ "' && Passwd='" + passwd + "'";
		int userID = 0;
		rs = DBManger.DataConn.executeQuery(sql2);
		try {
			while (rs.next()) {
				userID = rs.getInt(1);
	...
	public static String logIn(int userID) {
		...
		rs = DBManger.DataConn.executeQuery(sql);
		String usrname = null;
		try {
			while (rs.next()) {
	...
	public static int recordInformation(Information info, int status) {
		...
		String sql3="insert into User_info (ID,Sender,Receiver,Status,Time) values " +
