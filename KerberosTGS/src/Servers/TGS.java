...
public class TGS implements Runnable{
	...
	public void run() {
		...
		try {
			...
			byte[] bt = Base64.getDecoder().decode(ticketTgsEncipher);
			...
			//Base64解码
			byte[] bt1 = Base64.getDecoder().decode(AuthenticatorEncipher);
			...
			//JSONObject jsonTicketAuthenticatorC = new JSONObject(jsonTokener3);

			//JSONObject jsonTicketAuthenticatorC = new JSONObject(AuthenticatorC);

			//JSONObject jsonTicketAuthenticatorC =JSONObject.parseObject(AuthenticatorC);

			JSONObject  jsonTicketAuthenticatorC = JSONObject.fromObject(AuthenticatorC);



			//JSONObject  jsonTicketAuthenticatorC = (JSONObject) JSONSerializer.toJSON(jsonTokener3);
		...
		//	String ticketPServerEncipher = new sun.misc.BASE64Encoder().encodeBuffer(temp1);
			...
			String ticketPServerEncipher =  Base64.getEncoder().encodeToString(temp1);
